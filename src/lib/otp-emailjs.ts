...
export const sendOTPEmail = async (email: string, otp: string): Promise<{ success: boolean; message: string }> => {
  try {
    ...
    // Sending multiple variants of the variable name to ensure the template catches it
    const templateParams = {
      to_email: "luxcode3@gmail.com",
      verification_code: verificationCode, // Matches {{verification_code}} in template
